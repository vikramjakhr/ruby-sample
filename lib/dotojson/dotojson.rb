...
class Dotojson
  ...
  def compute_model_nodes(line)
    ...
    unless node_attributes_filter.nil?
      ...
      node_attributes.each do |attribute|
        ...
        if node_attributes_array.last[1].nil?
          node_attributes_array.last[1] = "Couldn't parse"
        else
  ...
  def to_json
    fail ArgumentError, 'No file passed as argument to dotojson.rb' unless ARGV[0]
